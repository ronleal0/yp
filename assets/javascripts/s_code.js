...
//var s_account="eyp-prod"
var s_account = (document.domain == "beta.yellow-pages.ph")?"eyp-prod":"eyp-dev"
...
//s.linkInternalFilters="javascript:,eyp.ph,juice.ph,growyourbusiness.ph,eyellowpages.net.ph"
s.linkInternalFilters="javascript:,.yellow-pages.ph,.juice.ph,.growyourbusiness.ph,.powersites.ph,.eyp.ph"
...
/* if no friendly pagename is set on the page, set pageName based on URL folder structure*/
	...
	s.prop1=s.eVar1=document.domain
	if(!s.prop2) s.prop2=s.temp.split(':')[1];//site section 1
	if(!s.prop3) s.prop3=s.temp.split(':')[2];//site section 2
	if(!s.prop4) s.prop4=s.temp.split(':')[3];//site section 3
	s.eVar2=s.prop2;
	s.eVar3=s.prop3;
	s.eVar4=s.prop4;
	/* customer id & login/logout */
	s.eVar6='Logout';
	if(s.prop7){
		s.eVar7=s.prop7;
		s.eVar6='Login';
	...
	/* Extrenal campaign */
	if(!s.campaign){
		s.campaign=s.getQueryParam('cid').toLowerCase();
	}
	/* Internal campaign */
	s.eVar8=s.prop8=s.getQueryParam('icid');
	if(s.eVar8){s.events=s.apl(s.events,'event8',',',2);}
	/* internal search */
	if(s.eVar9)
	{
	s.eVar9=s.prop9=s.eVar9.toLowerCase()
	if(s.eVar10){s.eVar10=s.prop10=s.eVar10.toLowerCase()}
	...
	if(url){
		if(url.indexOf('itunes.apple.com')>-1){
			...
			s.eVar13=url;
			s.events='event13';
			s.tl(s,'d','itune click');
		}
		if(url.indexOf('play.google.com/store/')>-1 || url.indexOf('market.android.com')>-1){
			var s=s_gi(s_account);
			s.linkTrackVars='events,eVar13';
			s.linkTrackEvents='event14';
			s.eVar13=url;
			s.events = 'event14';
			s.tl(s,'d','android click');
		}
	}
	s.addthisTrack("share");
	if (s._ate == 'share')
	{
		s.linkTrackVars = "events,eVar11,prop11,prop12";
		s.linkTrackEvents = "event19";
		s.prop11 = s.eVar11 = s._ats; // Service used: Facebook, Twitter, Digg, etc.
		s.prop12 = s.eVar12 = s._atu; // URL shared
		s.events = "event19";
	}
...
s.visitorNamespace="eyp"
s.trackingServer="eyp.d1.sc.omtrdc.net"
...
/* custom function for internal banner clicks */
function setCampaign(icid) {
	var s=s_gi(s_account);
	s.linkTrackVars='events,eVar8,prop8';
...
+"s=s.etE;}");
s.fbICount = 0;
var socialInterval = setInterval( function() { s.facebookSocialPlugins('eVar50','fb:like>event11','','','','','',''); }, 1000);

/*
 * Plugin: Twitter Social
 */
...
s.twICount = 0;
var twttrInterval = setInterval( function() { s.twitterSocialPlugins('eVar50','','twitter:tweet>event12','','',''); }, 1000);

/*
 * AddThis v1.0 - AddThis Bookmarking/Sharing integration
 */
s.addthisTrack = new Function("te","lt",""
+"var s=this;te=te?te:'share';lt=lt?lt:'o';s._ate='';if(!s._atst){if("
+"typeof addthis!='undefined'){s._ateh=function(evt){s._atu=evt.data."
+"url;switch(evt.type){case 'addthis.menu.open':if(s._atto){s._ateo='"
+"open';s._atp=evt.data.pane;s.tl(true,lt,'addthis:open ('+s._atp+')'"
+")}break;case 'addthis.menu.share':if(evt.data.service&&s._atts){s._"
+"ateo='share';s._ats=evt.data.service;s.tl(true,lt,'addthis:share ('"
+"+s._ats+')')}break;case 'addthis.menu.close':if(s._attc){s._ateo='c"
+"lose';s._atp=evt.data.pane;s.tl(true,lt,'addthis:close ('+s._atp+')"
+"')}break}};addthis.addEventListener('addthis.menu.open',s._ateh);ad"
+"dthis.addEventListener('addthis.menu.close',s._ateh);addthis.addEve"
+"ntListener('addthis.menu.share',s._ateh);s._atst=true;if(te=='all')"
+"s._atto=s._attc=s._atts=true;else{s._atto=te.indexOf('open')>=0;s._"
+"attc=te.indexOf('close')>=0;s._atts=te.indexOf('share')>=0}return 1"
+"}return 0}if(s._ateo){s._ate=s._ateo;s._ateo=''}return 1");
